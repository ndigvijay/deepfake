...
const App = () => {
    ...
    const predictVideo = async () => {
        try {
            ...
            const response = await axios.post(`${BASEURL}/predictVideo`, formData, {
                headers: {
    ...
    const predictImage = async () => {
        try {
            ...
            const response = await axios.post(`${BASEURL}/predictImage`, formData, {
                headers: {
                    'Content-Type': 'multipart/form-data',
                },
            });
                ...
                <div className="min-h-screen min-w-full flex flex-col gap-3 items-center justify-center p-4 bg-[#EBEEF5]">
                    ...
                    {videoUrl && (
                        <div className="w-11/12 sm:w-5/6 md:w-4/5 lg:w-3/5 xl:w-1/2 mt-32 flex items-center justify-center">
                            <video controls className="rounded-2xl" width="100%">
                    ...
                    {result && (
                        result.result === 0 ? (
                            <div className="text-2xl flex items-center gap-2">
                                Prediction Result: <span className="font-bold text-green-500">Real Face</span>
                            </div>
                        ) : result.result === 1 ? (
                            <div className="text-2xl flex items-center gap-2">
                                Prediction Result: <span className="font-bold text-red-500">Fake Face</span>
                            </div>
                        ) : (
                            <div className="text-2xl flex items-center gap-2">
                                Prediction Result: <span className="font-bold text-neutral-700">Error in Prediction</span>
                            </div>
                        )
                    )}
